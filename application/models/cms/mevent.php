<?php
	...
	*/
	class mevent extends CI_Model
	{
	    ...
	    {
	    	$sql = 'select event_detail from '.$this->table.' where id = '.$id;
	    	$query = $this->db->query($sql);
		...
		public function dropdownlist($active='')
		{
			# code...
			$html = '';
			$data = $this->getQuery($object="",$join="",$and="",$orderby="",$limit="");
			if($data){
				// $html .= '<option value="0">Select a item...</option>';
				foreach ($data as $key => $value) {
					# code...
					$selected = $active==$key ? 'selected':'';
					$html .= '<option '.$selected.' value="'.$value['id'].'">'.$value["event_name"].'</option>';						
				}
			}else{
