<?php
	...
	* 
	*/
	class mcompany_status extends CI_Model
	{
		...
		protected $table = "tkwp_company_status";
		/**begin danh sach*/
		...
		public function dropdownlist($active='')
		{
			# code...
			$html = '';
			$data = $this->getQuery("id,cst_name",$join="","","cst_order asc, id asc","");
			if($data){
				$html .= '<option value="0">-- Trình trạng làm việc --</option>';
				foreach ($data as $key => $value) {
					# code...
					$selected = $active==$value['id'] ? 'selected':'';
					$html .= '<option '.$selected.' value="'.$value["id"].'">'.$value["cst_name"].'</option>';						
				}
			}else{
