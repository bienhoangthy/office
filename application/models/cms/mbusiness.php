	...
	*/
	class mbusiness extends CI_Model
	{
		...
		public function dropdownlist($active='')
		{
			# code...
			$html = '';
			$data = $this->getQuery("id,business_name",$join="","","id","");
			if($data){
				$html .= '<option value="0">-- Chọn loại công ty --</option>';
				foreach ($data as $key => $value) {
					# code...
					$selected = $active==$value['id'] ? 'selected':'';
					$html .= '<option '.$selected.' value="'.$value["id"].'">'.$value["business_name"].'</option>';						
				}
			}else{
