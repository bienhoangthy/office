<?php
	...
	* 
	*/
	class maccount_name extends CI_Model
	{
		...
		protected $table = "tkwp_account_name";
		/**begin danh sach*/
		...
		public function dropdownlist($active='')
		{
			# code...
			$html = '';
			$data = $this->getQuery("id,ac_name",$join="","ac_status = 1","ac_orderby","");
			if($data){
				$html .= '<option value="0"> -- Chọn tài khoản -- </option>';
				foreach ($data as $key => $value) {
					# code...
					$selected = $active==$value['id'] ? 'selected':'';
					$html .= '<option '.$selected.' value="'.$value["id"].'">'.$value["ac_name"].'</option>';						
				}
			}else{
