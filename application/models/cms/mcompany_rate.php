<?php
	...
	* 
	*/
	class mcompany_rate extends CI_Model
	{
		...
		protected $table = "tkwp_company_rate";
		/**begin danh sach*/
		...
		public function dropdownlist($active='',$parent)
		{
			...
			$and = "";
			if ($parent != 0 && $parent != '') {
				$and = "rate_parent = ".$parent;
			}
			$data = $this->getQuery("id,rate_name",$join="",$and,"id","");
			if($data){
				$html .= '<option value="0">-- Loại khách hàng --</option>';
				foreach ($data as $key => $value) {
					# code...
					$selected = $active==$value['id'] ? 'selected':'';
					$html .= '<option '.$selected.' value="'.$value["id"].'">'.$value["rate_name"].'</option>';						
				}
			}else{
