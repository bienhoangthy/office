	...
	*/
	class mmenu extends CI_Model
	{
		...
		public function dropdownlist($parent='')
		{
			# code...
			$html = '';
			$data = $this->getQuery("id,menu_name",$join="","menu_parent = 1","menu_orderby","");
			if($data){
				$html .= '<option value="0">Select a item...</option>';
				foreach ($data as $key => $value) {
					# code...
					$selected = $parent==$value['id'] ? 'selected':'';
					$html .= '<option '.$selected.' value="'.$value["id"].'">'.$value["menu_name"].'</option>';	
					$data2 = $this->getQuery("id,menu_name",$join="","menu_parent = ".$value['id'],"menu_orderby","");
					if($data2){
						foreach ($data2 as $key2 => $value2) {
							# code...
							$selected = $parent==$value2['id'] ? 'selected':'';
							$html .= '<option '.$selected.' value="'.$value2["id"].'">-- '.$value2["menu_name"].'</option>';	
						}
					}
				}
			}else{
