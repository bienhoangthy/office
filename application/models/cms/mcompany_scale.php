<?php
	...
	* 
	*/
	class mcompany_scale extends CI_Model
	{
		...
		protected $table = "tkwp_company_scale";
		/**begin danh sach*/
		...
		public function dropdownlist($active='')
		{
			# code...
			$html = '';
			$data = $this->getQuery("id,scale_name",$join="","","id","");
			if($data){
				$html .= '<option value="0">-- Quy mô công ty --</option>';
				foreach ($data as $key => $value) {
					# code...
					$selected = $active==$value['id'] ? 'selected':'';
					$html .= '<option '.$selected.' value="'.$value["id"].'">'.$value["scale_name"].'</option>';						
				}
			}else{
