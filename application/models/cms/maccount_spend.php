<?php
	...
	* 
	*/
	class maccount_spend extends CI_Model
	{
		...
		protected $table = "tkwp_account_spend";
		/**begin danh sach*/
		...
		public function dropdownlist($active='')
		{
			# code...
			$html = '';
			$data = $this->getQuery("id,spend_name",$join="","spend_parent = 0","spend_orderby","");
			if($data){
				$html .= '<option value="0">Select a item...</option>';
				foreach ($data as $key => $value) {
					# code...
					$selected = $active==$value['id'] ? 'selected':'';
					$html .= '<option '.$selected.' value="'.$value["id"].'">'.$value["spend_name"].'</option>';	
					$data2 = $this->getQuery("id,spend_name",$join="","spend_parent = ".$value['id'],"spend_orderby","");
					if($data2){
						foreach ($data2 as $key2 => $value2) {
							# code...
							$selected = $active==$value2['id'] ? 'selected':'';
							$html .= '<option '.$selected.' value="'.$value2["id"].'">-- '.$value2["spend_name"].'</option>';	
						}
					}
				}
			}else{
