<?php
	...
	* 
	*/
	class mcompany_work_status extends CI_Model
	{
		...
		protected $table = "tkwp_company_work_status";
		/**begin danh sach*/
		...
		public function dropdownlist($active='')
		{
			# code...
			$html = '';
			$data = $this->getQuery("id,wk_name",$join="","","id","");
			if($data){
				$html .= '<option value="0">-- Chọn trạng thái --</option>';
				foreach ($data as $key => $value) {
					# code...
					$selected = $active==$value['id'] ? 'selected':'';
					$html .= '<option '.$selected.' value="'.$value["id"].'">'.$value["wk_name"].'</option>';						
				}
			}else{
