                        ...
                        <div class="panel panel-primary">
                            ...
                            <div class="panel-toolbar-wrapper pl0 pt5 pb5">
                                ...
                                <div class="panel-toolbar text-right">
                                    ...
                                    <a href="<?= my_lib::cms_site()?>translate/add/" class="btn btn-sm btn-primary"><i class="ico-plus"></i> Add</a>
                                </div>
                            </div>
                            <!--/ panel toolbar wrapper -->
                                ...
                                <table class="table table-bordered table-hover" id="table1">
                                    <thead>
                                        <tr>
                                            ...
                                            {
                                                foreach ($translate_lang as $key => $lang) {
                                                    # code...
                                            ...
                                            <th class="text-center" width="10%">Thuộc loại</th>
                                    ...
                                    <tbody>
                                        ...
                                        if(isset($list) && $list){
                                            ...
                                            foreach ($list as $key => $value) {
                                                ...
                                                $tmp_stautus = $this->mtranslate->listTypeName($value['translate_type']);                                                
                                                $value['translate_type'] = '<span class="label label-success">'.$tmp_stautus["name"].'</span>';                                                
                                                $link_view = my_lib::cms_site().'translate/detail/'.$value["id"].'/?redirect='.base64_encode(current_url());
                                                $link_update = my_lib::cms_site().'translate/edit/'.$value["id"].'/?redirect='.base64_encode(current_url());
                                                $link_delete = my_lib::cms_site().'translate/delete/'.$value["id"].'/?redirect='.base64_encode(current_url());;
                                                echo '<tr>';
                                                    echo '<td>';
                                                        ...
                                                        echo '</div>';
                                                    echo '</td>';                                                                                                    
                                                    echo '<td><a href="'.$link_view.'">'.$value["translate_name"].'</a><br /><code>'.$value["translate_code"].'</code></td>';
                                                    if(isset($translate_lang) && $translate_lang)
                                                    {
                                                        foreach ($translate_lang as $key => $lang) {
                                                            # code...
                                                            $getTranslateLang = '';
                                                            $getTranslateLang = $this->mtranslate_lang->getData('',array("translate_id"=>$value["id"],"translate_lang"=>$lang["lang"]));
                                                            $getTranslateLang["translate_name"] = isset($getTranslateLang["translate_name"]) && $getTranslateLang["translate_name"] ? $getTranslateLang["translate_name"] : '';
                                                            echo '<td>'.$getTranslateLang["translate_name"].'</td>';
                                                        }
                                                    }                                                    
                                                    echo '<td>'.$value["translate_create_date"].'</td>';                                                    
                                                    echo '<td class="text-center">'.$value["translate_type"].'</td>';
